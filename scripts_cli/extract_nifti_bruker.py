...
def calc_nii(in_dirpath, compress=True, force=False, verbose=D_VERB_LVL):
    ...
    if os.path.exists(data_filepath):
        if mrt.utils.check_redo(in_filepath_list, out_filepath_list, force):
...
def extract_nii(dirpath, extradir, force, verbose):
    ...
    TODO: integrate with mrt.utils.check_redo()
    """
    ...
    if all([os.path.exists(filepath) for filepath in filepath_list]):
        ...
        info_dict = mrt.naming.parse_filename('')
        ...
        if all([method_ldr in method_ldr_dict for method_ldr
            ...
            old_phs_filepath_list = mrt.input_output.split(phs_filepath)
            ...
            for i, (mt_freq, mt_power, old_mag_filepath, old_phs_filepath) \
                ...
                info_dict['protocol'] = mrt.naming.to_protocol(
                    base_protocol, param_dict)
                info_dict['img_type'] = mrt.naming.TYPE_ID['mag']
                new_mag_filepath = mrt.naming.to_filename(info_dict, dest_dirpath)
                info_dict['img_type'] = mrt.naming.TYPE_ID['phs']
                new_phs_filepath = mrt.naming.to_filename(info_dict, dest_dirpath)
                postprocess_nii_mag(
...
def extract_nifti(dirpath, extradir, force, verbose):
    ...
    """
    in_dirpath_list = mrt.utils.listdir(dirpath, None)
    for in_dirpath in in_dirpath_list:
        in_subdirpath_list = mrt.utils.listdir(in_dirpath, None)
