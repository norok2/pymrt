...
def _get_shape(base_shape, *extras):
    extras = tuple(fc.auto_repeat(extras, 1))
...
def _load_bin(
        ...
        cx_interleaved=True,
        dry=False):
    byte_size = struct.calcsize(fc.DTYPE_STR[dtype])
    with fc.zopen(filepath, 'rb') as file_obj:
        file_size = file_obj.seek(0, 2)
        file_obj.seek(0)
        if not dry:
            if user_filter:
                raise NotImplementedError
            arr = np.array(fc.read_stream(
...
def _to_patterns(name, exts):
    return [fc.change_ext('*/' + name, ext) for ext in exts]


# ======================================================================
def _get_single(dirpath, name, exts):
    filepaths = fc.flistdir(_to_patterns(name, exts), dirpath)
    if len(filepaths) == 1:
...
def _get_scan_num_sample_id(comments):
    ...
    source_filepath = fc.multi_split_path(lines[0])
...
def _reco_from_fid(
    ...
    if is_cartesian:
        ...
        dtype_size = struct.calcsize(
            load_info['mode'] + fc.DTYPE_STR[load_info['dtype']])
        ...
        try:
            ...
            fid_shape = (
                fc.align(base_shape[0], block_size // num_coils),
                num_coils,
                num_images,
                fc.align(base_shape[1], pe_factor, 'lower'),
